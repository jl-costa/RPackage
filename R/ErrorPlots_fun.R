...
#'@return an 'error_bars2.out' object which contains (i) a dataframe of useful summary variables (ii) a dot plot showing the error bars, and (iii) a copy of the original dataframe with the bin_x variable appended as a third column.
#'@export



error_bars2 = function(df, s = NULL, draw='both'){
  ...
  if(!"package:ggplot2" %in% search())
    print('ggplot2 is now loaded in the environment. Rerun the command to use the error_bars2() function')
  library(ggplot2)
  ...
  if(!"package:dplyr" %in% search())
    print('dplyr is now loaded in the environment. Rerun the command to use the error_bars2() function')
  library(dplyr)
  
  if(!"package:data.table" %in% search())
    print('data.table is now loaded in the environment. Rerun the command to use the error_bars2() function')
  library(data.table)
  ...
  if (colnames(df)[1] != 'x' | colnames(df)[2] != 'y') {
    stop("df columns must be labeled correctly and in in the following order: (x,y)")
  }
  ...
  # create summary df and process it as a data.table object
  
  df.summary <- data.table(df.copy)
  
  df.summary <- df.summary[,list(xmean= mean(x),
                                   ymin= min(y),
                                   ymax = max(y),
                                   ymean = mean(y),
  ...
  # Specify dplyr:: in case of conflict with plyr::count()
  
  df.summary$ynum = dplyr::count(df.copy, vars = bin_x)$n
  ...
  error_bars2.out = structure(list(plot = errorbars_plot,
                                  df.summary = df.summary,
                                  df = df.copy),
                             class = 'error_bars2.object')
  
  return(error_bars2.out)
}
